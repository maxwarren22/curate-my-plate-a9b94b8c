...
function generateHtml(meals: Meal[], shoppingList: string, type: 'full' | 'shopping') {
  ...
  if (type === 'shopping') {
    ...
    return `<html><body><h1>Shopping List for week of ${weekStart}</h1><ul>${shoppingListItems}</ul></body></html>`;
  }
  ...
  return `<html><body><h1>Your Meal Plan for the week of ${weekStart}</h1>${mealCards}</body></html>`;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: mealHistory, error: mealsError } = await supabaseClient.from('user_meal_history').select(`meal_date, total_time_to_cook, main_dish:main_dish_recipe_id(*), side_dish:side_dish_recipe_id(*)`)
      .eq('user_id', user.id).gte('meal_date', weekStartDate.toISOString().split('T')[0]).lte('meal_date', weekEndDate.toISOString().split('T')[0]);
    ...
    const allIngredients = meals.flatMap(m => [...(m.main_dish.ingredients?.split('\n') || []), ...(m.side_dish.ingredients?.split('\n') || [])]).filter(Boolean);
    
    const parsedIngredients = allIngredients.flatMap(ing => parseIngredient(ing));
    ...
    // --- FIX: Use a specific type for the mapped values ---
